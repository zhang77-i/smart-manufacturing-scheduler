...
面向带释放时间、机器兼容性、机器相关加工时长、机器最早可用时间和到达后 24 小时硬截止的异构并行机排程案例研究；可复现建模层额外支持可选的订单前后置关系。

## 建模主线

- 每个订单选择一台兼容机器，或外发；
- 厂内订单满足释放时间、机器可用时间、硬截止与同机不重叠；
- 同时在厂内加工的前置与后续订单满足完工—开工顺序；
...
示例 JSON 使用可选的 `predecessors` 字段演示前后置约束。887 单全规模聚合结果仍对应独立订单排程，不把这个新增通用能力追溯写入原案例。
